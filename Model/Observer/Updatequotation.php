...
class Updatequotation implements ObserverInterface
{
        ...
        try {
            ...
            if ($collection->count()) {
                ...
                if (isset($data['delivery_day'])) {
                    $quotation->setDeliveryDay($data['delivery_day']);
                }
                $quotation->save();
    ...
    public function sendEmail($quotation)
    {
        ...
        /* Sender Detail  */
        $senderInfo = [
            'name' => $this->_quotationHelper->getAdminName(),
            'email' => $this->_quotationHelper->getAdminEmail(),
        ];
        ...
        $emailTempVariables['quotation_status'] = $this->_quotationHelper
                ->getQuotationStatus($quotation->getQuotationStatus());
        
        $adminEmailTemplateVariables = [];
        $adminEmailTemplateVariables['admin_name'] = $this->_quotationHelper->getAdminName();
        $adminEmailTemplateVariables['quotation_status'] = $emailTempVariables['quotation_status'];
        ...
        $this->_quotationHelper->sendMail(
                $adminEmailTemplateVariables, $senderInfo, $senderInfo, false
        );
    }
