...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    const ADMIN_NAME = 'quotation_configuration/general/quotation_admin_name'; //ADMIN NAME
    ...
    public function getProductImaege($pid = null)
    {
        if ($pid) {
            return $this->_productImage
                    ->init($this->_product->load($pid), 'category_page_list')
                    ->constrainOnly(false)
                    ->keepAspectRatio(TRUE)
                    ->keepFrame(false)->resize(400)->getUrl();
        }
    }
    
    public function getConfigurableProductImaege($pid=null ,$options = null){
        if($pid && $options){
            ...
            foreach ($simple as $sProduct) {
                ...
                foreach ($attributes as $attribute) {
                    ...
                    $att[$attribute->getLabel()] = $attribute
                                    ->getProductAttribute()->getFrontend()->getValue($sProduct);
                ...
                if ($options == $confArray) {
                    return ['img'=>$this->getProductImaege($sProduct->getId()),'att'=>$att];
                    //return $att;
                }
            }
        }
    ...
    /*public function getConfigurableProductOptions($pId = null, $options = null)
    {
        $product = $this->_product->load($pId);
        if ($product && $options && $product->getTypeId() == 'configurable') {
            $attributes = $product->getTypeInstance(true)->getConfigurableAttributes($product);
